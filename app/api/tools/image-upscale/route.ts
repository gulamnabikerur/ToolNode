...
export const POST = createApiHandler({
  handler: async (req) => {
    ...
    if (file.size > maxSize) return NextResponse.json({ error: "File too large (max 8MB)" }, { status: 400 });

    const buffer = Buffer.from(await file.arrayBuffer());
    const hfToken = process.env.HUGGINGFACE_API_TOKEN;

    if (!hfToken || hfToken === "hf_demo_placeholder") {
      return NextResponse.json({ error: "HuggingFace API token not configured. Add HUGGINGFACE_API_TOKEN to .env.local", demo: true }, { status: 503 });
    }
